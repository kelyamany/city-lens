...
API_KEY = "AIzaSyAeXbtdoSFHSWF1B75BvAtt0-R_K6WgO7c"
address = "1600 Amphitheatre Parkway, Mountain View, CA"

# 1. Geocode address to get place_id
geo_url = f"https://maps.googleapis.com/maps/api/geocode/json?address={address}&key={API_KEY}"
geo_resp = requests.get(geo_url).json()

if not geo_resp.get("results"):
    print("No results found for the address. Full response:", geo_resp)
    exit(1)

# 2. Define place types to search for
place_types = [
    "cafe", "park", "store", "restaurant", "bar", "museum",
    "shopping_mall", "gym", "library", "hotel", "tourist_attraction"
]

# 3. Get lat/lng from geocoding result
location = geo_resp["results"][0]["geometry"]["location"]
lat, lng = location["lat"], location["lng"]

# 4. For each type, search for up to 2 places nearby and print their name and first review
